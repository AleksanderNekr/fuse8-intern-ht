...
public class AccountProcessorBenchmarks
{
    ...
    /// | CalculatePerformed |   243.6 ns |  3.04 ns |  2.84 ns |    1 |      - |         - |
    ///
    /// Как видно из результатов, улучшенная версия работает более чем в 2 раза быстрее, чем стандартная, так как
    /// с помощью ключевого слова `ref` происходит передача по ссылке структуры, а не ее копирование, как
    ...
    public class AccountProcessorBenchmark
    {
        ...
        {
            return AccountProcessor.CalculatePerformed(ref this._bankAccount);
        }
    }
